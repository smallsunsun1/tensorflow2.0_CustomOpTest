...
class PrefetchDatasetOp::Dataset : public DatasetBase {
 public:
  explicit Dataset(OpKernelContext *ctx, const DatasetBase *input, int64 buffer_size,
                   int64 slack_period, bool legacy_autotune) : DatasetBase(DatasetContext(ctx)),
                                                               input_(input),
  ...
  std::unique_ptr<IteratorBase> MakeIteratorInternal(const std::string &prefix) const override {
      return absl::make_unique<Iterator>(Iterator::Params{this,
  ...
  class Iterator : public DatasetIterator<Dataset> {
   public:
    ...
    Status Initialize(IteratorContext *ctx) override {
        ...
        TF_RETURN_IF_ERROR(RegisterCancellationCallback(ctx->cancellation_manager(),
                                                        [this]() { CancelThreads(); }, &deregister_fn_));
    ...
    Status GetNextInternal(IteratorContext *ctx, std::vector<Tensor> *out_tensors,
        ...
        const auto &stats_aggregator = ctx->stats_aggregator();
        {
            ...
            if (legacy_autotune_) {
                ...
                while (!cancelled_ && buffer_.empty() && !prefetch_thread_finished_ && buffer_size_->value != 0) {
                    RecordStop(ctx);
                    cond_var_->wait(l);
                    RecordStart(ctx);
                }
        ...
        mutex_lock input_l(input_mu_);
        {
            ...
            if (stats_aggregator) {
                ...
                stats_aggregator->AddScalar(stats_utils::BufferCapacityScalarName(dataset()->node_name()),
   ...
   protected:
    std::shared_ptr<model::Node> CreateNode(IteratorContext *ctx, model::Node::Args args) const override {
        return model::MakeAsyncKnownRatioNode(std::move(args), 1, {model::MakeParameter(kBufferSize,
                                                                                        buffer_size_,
                                                                                        0,
    ...
    Status SaveInternal(IteratorStateWriter *writer) override {
        ...
        TF_RETURN_IF_ERROR(writer->WriteScalar(full_name(kBufferSize), buffer_.size()));
   ...
   private:
    ...
    Status Consume(IteratorContext *ctx, std::vector<Tensor> *out_tensors,
        ...
        if (stats_aggregator) {
            ...
            stats_aggregator->AddToHistogram(stats_utils::BufferUtilizationHistogramName(dataset()->node_name()),
                                             {static_cast<float>(buffer_.size() /
                                                 static_cast<float>(buffer_limit_))},
                                             num_elements());
            stats_aggregator->AddScalar(stats_utils::BufferSizeScalarName(dataset()->node_name()),
                                        static_cast<float>(buffer_.size()), num_elements());
            stats_aggregator->AddScalar(stats_utils::BufferCapacityScalarName(dataset()->node_name()),
        ...
        if (s.ok()) {
            if (dataset()->slack_period_ > 0 && (num_elements() + 1) % dataset()->slack_period_ == 0) {
                int64 slack_us = EnvTime::Default()->NowMicros() - buffer_.front().created_us;
                slack_us_ = kSleepFactor * slack_us + slack_us;
    ...
    Status EnsurePrefetchThreadStarted(IteratorContext *ctx) EXCLUSIVE_LOCKS_REQUIRED(*mu_) {
        if (!prefetch_thread_) {
            std::shared_ptr<IteratorContext> new_ctx = std::make_shared<IteratorContext>(*ctx);
            prefetch_thread_ = ctx->StartThread("tf_data_prefetch", [this, new_ctx]() {
              PrefetchThread(new_ctx);
            });
        }
        return Status::OK();
    }
    void PrefetchThread(const std::shared_ptr<IteratorContext> &ctx) {
        ...
        while (true) {
            ...
            if (dataset()->slack_period_ > 0 && num_produced % dataset()->slack_period_ == 0) {
                // For the first element in the "burst", sleep for a bit if there is slack
            ...
            buffer_element.status = input_impl_->GetNext(ctx.get(), &buffer_element.value, &end_of_sequence);
            ...
            // 3. Signal that the element has been produced.
            {
                ...
                buffer_element.created_us = EnvTime::Default()->NowMicros();
    ...
    Status WriteStatus(IteratorStateWriter *writer, size_t index,
        ...
        TF_RETURN_IF_ERROR(writer->WriteScalar(CodeKey(index), static_cast<int64>(status.code())));
        if (!status.ok()) {
    ...
    Status ReadStatus(IteratorStateReader *reader, size_t index, Status *status) {
        ...
        if (code != error::Code::OK) {
            ...
            TF_RETURN_IF_ERROR(reader->ReadScalar(ErrorMessageKey(index), &error_message));
    ...
    string ErrorMessageKey(size_t index) {
        return full_name(strings::StrCat(kStatus, "[", index, "]", kErrorMessageSuffix));
    }
...
PrefetchDatasetOp::PrefetchDatasetOp(tensorflow::OpKernelConstruction *ctx):UnaryDatasetOpKernel(ctx) {
...
void PrefetchDatasetOp::MakeDataset(tensorflow::OpKernelContext *ctx,
    ...
    *output = new Dataset(ctx, input, buffer_size, slack_period_, legacy_autotune_);
}

namespace {
REGISTER_KERNEL_BUILDER(Name("CustomPrefetchDataset").Device(DEVICE_CPU).Priority(2),
                        PrefetchDatasetOp);
REGISTER_KERNEL_BUILDER(Name("CustomPrefetchDataset")
                            .Device(DEVICE_GPU)
